    ...
    public function index(Request $request)
{
    ...
    // ========================
    // DATA SISWA
    // URUT KELAS, LALU NAMA A-Z
    // ========================
    $siswa = $query
        ->orderBy(
            Kelas::select('nama_kelas')
                ->whereColumn('kelas.id_kelas', 'siswa.id_kelas'),
            'asc'
        )
    ...
    $kelas = Kelas::orderByRaw(
            'CAST(LEFT(nama_kelas, 1) AS UNSIGNED) ASC'
        )
    ...
    public function kenaikanKelas()
    {
        ...
        try {
            ...
            // ==========================
            $kelas6 = Kelas::where('nama_kelas', 'like', '6%')->pluck('id_kelas');
            ...
            // ==========================
            // NONAKTIFKAN WALI
            // JIKA TIDAK PUNYA SISWA AKTIF
            // ==========================
            foreach ($siswaLulus as $siswa) {
                $relasi = SiswaWali::where('id_siswa', $siswa->id_siswa)->first();
                if ($relasi) {
                    $masihAdaSiswaAktif = SiswaWali::join('siswa', 'siswa.id_siswa', '=', 'siswa_wali.id_siswa')
                    ...
                    if (!$masihAdaSiswaAktif) {
                        Wali::where('id_wali', $relasi->id_wali)->update(['is_active' => 0]);
                        $userId = Wali::where('id_wali', $relasi->id_wali)->value('id_user');
                        User::where('id', $userId)->update(['status' => 'nonaktif']);
                    }
                }
            }

            // ==========================
            // NONAKTIFKAN SISWA KELAS 6
            // ==========================
            Siswa::whereIn('id_kelas', $kelas6)->update([
                'status' => 'lulus',
                'is_active' => 0,
            ]);
            ...
            foreach ($mapping as $asal => $tujuan) {
                ...
                $dataKenaikan[] = [
                    'siswa_ids' => Siswa::where('id_kelas', $kelasAsal->id_kelas)->pluck('id_siswa'),
            ...
            foreach ($dataKenaikan as $data) {
                Siswa::whereIn('id_siswa', $data['siswa_ids'])->update([
            ...
            if ($siswa2D->count() > 0) {
                ...
                $tujuan = [$kelas3A->id_kelas, $kelas3B->id_kelas, $kelas3C->id_kelas];
                $index = 0;
            ...
            DB::commit();
            return redirect()->route('data-siswa')->with('success', 'Kenaikan kelas berhasil diproses.');
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->route('data-siswa')->with('error', $e->getMessage());
        }
    }
    ...
    public function arsipkan(Request $request, $id)
    {
        ...
        // CARI WALI TERHUBUNG
        // ========================
        $waliIds = DB::table('siswa_wali')->where('id_siswa', $siswa->id_siswa)->pluck('id_wali');

        // ========================
        // NONAKTIFKAN WALI
        // ========================
        DB::table('wali')->whereIn('id_wali', $waliIds)->update(['is_active' => 0]);

        return back()->with('success', 'Data siswa berhasil diarsipkan');
    }
}
