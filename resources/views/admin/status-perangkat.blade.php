    ...
    <div class="container-dashboard">
        ...
        <div class="row g-3 mb-3">
            @foreach($perangkat as $p)
                @php $status = strtolower(trim($p->status_koneksi ?? 'offline')); @endphp
                <div class="col-md-4">
                    <div class="status-card">
                        <div class="device-header">
                            <div class="device-icon"><i class="fa-solid fa-microchip"></i></div>
                            <div>
                                <h6 class="device-name">{{ $p->nama_device ?? 'RFID Reader' }}</h6>
                                <span class="device-type">Perangkat IoT</span>
                            </div>
                        </div>
                        <div class="device-info">
                            ...
                            <div class="info-item">
                                ...
                                <span class="info-value">{{ $p->ip ?? '-' }}</span>
                            </div>
                        </div>
                        <div class="last-active">
                            <i class="fa-regular fa-clock"></i> Terakhir aktif: {{ $p->last_active ?? '-' }}
                        </div>
                    </div>
                </div>
            @endforeach
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        @foreach($logs as $log)
                        <tr>
                            <td>{{ Carbon::parse($log->created_at)->format('H:i:s') }}</td>
                            <td>{{ $log->id_device == 2 ? $log->fingerprint_id : $log->uid_rfid }}</td>
                            <td>{{ $log->id_device == 2 ? ($log->nama_wali ?? '-') : ($log->nama_siswa ?? '-') }}</td>
                            <td>{{ $log->id_device == 2 ? 'Fingerprint' : 'RFID' }}</td>
                            <td>{{ $log->peran }}</td>
                            <td>
                                <span class="badge {{ strtolower($log->status ?? 'berhasil') === 'gagal' ? 'bg-danger' : 'bg-success' }}">
                                    {{ strtolower($log->status ?? 'berhasil') === 'gagal' ? 'Gagal' : 'Berhasil' }}
                                </span>
                            </td>
                        </tr>
                        @endforeach
