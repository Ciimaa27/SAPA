            ...
            <form action="{{ route('kelola-akun.store') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <select name="peran" id="peran" class="form-select @error('peran') is-invalid @enderror" required>
                        ...
                        <option value="Orangtua/Wali" {{ old('peran') == 'Orangtua/Wali' ? 'selected' : '' }}>Orang Tua / Wali</option>
                        <option value="Kepala Sekolah" {{ old('peran') == 'Kepala Sekolah' ? 'selected' : '' }}>Kepala Sekolah</option>
                    </select>
                    <small class="text-muted d-block mt-1">*Pilih peran akun yang akan dibuat.</small>
                    @error('peran')
                ...
                <div class="row mb-4">
                    <div class="col-md-6 mb-3 mb-md-0">
                        ...
                        <input type="text" id="nama_lengkap" name="nama_lengkap" value="{{ old('nama_lengkap') }}" class="form-control @error('nama_lengkap') is-invalid @enderror" placeholder="Masukkan nama lengkap..." required>
                        <small class="text-muted d-block mt-1">*Digunakan untuk peran admin dan kepsek</small>
                        @error('nama_lengkap')
                    ...
                    <div class="col-md-6">
                        <label class="form-label fw-semibold">Pilih Guru / Wali</label>
                        <select id="data_user" name="data_user" class="form-select" disabled>
                            <option value="">Pilih Data</option>
                        </select>
                        <small class="text-muted d-block mt-1">*Digunakan untuk memilih data Guru atau Orang Tua/Wali yang sudah terdaftar.</small>
                    </div>
                </div>

                <div class="mb-4">
                    <label class="form-label fw-semibold">Nama Pengguna</label>
                    <input type="text" name="username" value="{{ old('username') }}" class="form-control @error('username') is-invalid @enderror" placeholder="Masukkan nama pengguna..." required>
                    <small class="text-muted d-block mt-1">*Digunakan untuk login.</small>
                    @error('username')
                        <small class="text-danger d-block mt-1">{{ $message }}</small>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" class="form-control @error('email') is-invalid @enderror" placeholder="Masukkan email..." required>
                    <small class="text-muted d-block mt-1">*Pastikan email masih aktif.</small>
                    @error('email')
                        <small class="text-danger d-block">{{ $message }}</small>
                    @enderror
                </div>
                ...
                <div class="row mb-4">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="input-group">
                            <input type="password" id="password_confirmation" name="password_confirmation" class="form-control" placeholder="Masukkan kembali password..." required>
                            <button type="button" class="btn btn-outline-secondary" id="toggleConfirmPassword">
                                <i class="fas fa-eye"></i>
                            </button>
                        </div>
                        <small class="text-muted d-block mt-1">*Masukkan password yang sama.</small>
                    </div>
                </div>
    ...
    peran.addEventListener('change', function () {
        ...
        if (this.value === 'Guru') {
            ...
            guru.forEach(function(item) {
                dropdown.innerHTML += `<option value="${item.id_guru}">${item.nama_guru}</option>`;
            });
        }
            ...
            wali.forEach(function(item) {
                dropdown.innerHTML += `<option value="${item.id_wali}">${item.nama_wali}</option>`;
            });
        }
        else {
